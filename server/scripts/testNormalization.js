...
function normalizeBranch(rawBranch) {
  ...
  // 1. Strip address/location prefixes using START_KEYWORDS (run after degree prefix removal)
  let earliestIndex = -1;
...
const originalBranches = [...new Set(cutoffs.map(item => item.branch))];
const normalizedBranches = [...new Set(cutoffs.map(item => normalizeBranch(item.branch)).filter(b => b.length > 0))].sort();

console.log("Original Unique Branches:", originalBranches.length);
console.log("Normalized Unique Branches:", normalizedBranches.length);

console.log("\n--- Normalized Branches List ---");
console.log(normalizedBranches);
console.log("Total unique:", normalizedBranches.length);